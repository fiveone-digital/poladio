...
import 'edit_walkin_model.dart';
export 'edit_walkin_model.dart';

class EditWalkinWidget extends StatefulWidget {
  const EditWalkinWidget({
    Key? key,
  ...
  @override
  _EditWalkinWidgetState createState() => _EditWalkinWidgetState();
}

class _EditWalkinWidgetState extends State<EditWalkinWidget>
    with TickerProviderStateMixin {
  late EditWalkinModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EditWalkinModel());

    // On page load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.apiResult83a = await PoladioAPIsGroup.viewWalkInCall.call(
      ...
      if ((_model.apiResult83a?.succeeded ?? true)) {
        setState(() {});
      }
    });
                        ...
                        children: [
                          Flexible(
                            child: FFButtonWidget(
                              onPressed: () async {
                                _model.bookingCreate = await PoladioAPIsGroup
                                    .updateWalkInCall
                                    .call(
                                  token: FFAppState().token,
                                  projectId: getJsonField(
                                  ...
                                  enquiryAt: _model.enquiryValue?.toString(),
                                  srNo:
                                      int.tryParse(_model.srNoController.text),
                                  ...
                                  interestedIn: _model.interestedInValue,
                                  budget: _model.budgetValue,
                                  locationPreferred:
                                      _model.locPrefController.text,
                                  source: _model.sourceValue,
                                  reasonForPurchase: _model.reasonPurchaseValue,
                                  officeAddress:
                                      _model.officeAddressController.text,
                                  website: _model.websiteController.text,
                                  exisitingBookingName:
                                      _model.existingNameController.text,
                                  id: getJsonField(
                                    widget.walkinDetail,
                                    r'''$.id''',
                                  ),
                                  channelMobile: int.tryParse(
                                      _model.channelMobileController.text),
                                  channelName:
                                      _model.channelPartnerNameController.text,
                                  channelAddress:
                                      _model.channelAddressController.text,
                                  channelCompany:
                                      _model.channelCompanyNameController.text,
                                );
                                if ((_model.bookingCreate?.succeeded ?? true)) {
                                  setState(() {
                                    _model.currentTab = 4;
                                  });
                                }
